...
두 인자는 Lodash와 같은 방식으로 변환돼요. `number`는 숫자로, `precision`은 정수로 변환해요. 그래서 `round('4.016', '2')`도 동작하고, 심볼은 `NaN`이 되고, `true`는 자리수 `1`로 취급되고, `292`를 넘는 `precision`은 `292`로 제한돼요.
